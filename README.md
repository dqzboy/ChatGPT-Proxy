## 一、为啥需要自建反代
目前openai提供两种访问方式，一直是直接chatGPT WEB端访问的方式，也就是access token方式只要能登入就可以使用，并且是免费使用GPT-3.5模型，缺点就是不稳定并且无法扩展；另外一种就是API，注册用户目前赠送`5$`，使用完就需要付费了，优点就是稳定，但是缺点`5$`比较少并且限流，目前是3条/分钟。所以对于白嫖用户来说，肯定是选择使用access token的方式，但是目前唯一要解决的就是不稳定以及IP容易被ban的这个问题，所以需要我们自建反代服务来解决这些问题，公共的反代很不稳定，毕竟免费共享出来供大家使用的嘛。
