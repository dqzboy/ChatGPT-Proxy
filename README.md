...
- 可以访问到openai地址;或者国内服务器实现科学上网也可以
  ...
  - 目前个人使用的机场：[机场1按量不限时](https://mojie.la/#/register?code=CG6h8Irm)，[机场2按周期](https://teacat.cloud/#/register?code=ps4sZcDa)
...
## 四、项目使用自建反代
> 现在我们可以找一个项目，使用access token模式，并使用我们自建的代理地址进行访问；自建IP的访问地址为http://<vps-ip>:8080/conversation；如果项目与反代服务同在一台VPS上，则反代地址直接http://127.0.0.1:8080/conversation；如果你前端项目是容器启的，配置反代：http://go-chatgpt-api:8080/conversation
